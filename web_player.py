...
class WebPlayer:
    def __init__(self, game_dir, name, is_mafia):
    ...
    def get_new_messages(self):
        ...
        if manager_file.exists():
            with open(manager_file, 'r') as f:
                lines = f.readlines()[self.num_read_manager:]
                self.num_read_manager += len(lines)
                for line in lines:
                    messages.append({'text': line.strip(), 'type': 'manager'})
        
        # Daytime chat messages
        daytime_file = self.game_dir / PUBLIC_DAYTIME_CHAT_FILE
        if daytime_file.exists():
            with open(daytime_file, 'r') as f:
                lines = f.readlines()[self.num_read_daytime:]
                self.num_read_daytime += len(lines)
                for line in lines:
                    messages.append({'text': line.strip(), 'type': 'chat'})
    ...
    def send_message(self, message):
        ...
        with open(self.personal_chat_file, 'a') as f:
            f.write(formatted)
    
    def send_vote(self, voted_name):
        """Send a vote"""
        with open(self.personal_vote_file, 'a') as f:
            f.write(voted_name + '\n')
...
def join_game():
    ...
    # Get player name from real name
...
def get_messages():
    ...
    # Get remaining players for voting
    remaining = []
    if can_vote:
        remaining_file = player.game_dir / REMAINING_PLAYERS_FILE
        remaining = remaining_file.read_text().splitlines()
